...
function isItemAllowed(item: MenuItem, role: SchoolRole | undefined): boolean {
...
function filterMenu(
  ...
  return collapseAdjacentHeadings(result);
}

// Drop headings that no longer have any items beneath them after role filtering.
function collapseAdjacentHeadings(items: MenuConfig): MenuConfig {
  const out: MenuConfig = [];
  for (let i = 0; i < items.length; i++) {
    const item = items[i];
    if (item.heading) {
      const next = items[i + 1];
      if (!next || next.heading) continue;
    }
    out.push(item);
  }
  return out;
...
export function SidebarMenu() {
  ...
  const [isImpersonating, setIsImpersonating] = useState(
    () => impersonation.get() !== null,
  );

  useEffect(
    () =>
      impersonation.subscribe((state) => setIsImpersonating(state !== null)),
    [],
  );

  const visibleMenu = useMemo(
    () => filterMenu(MENU_SIDEBAR, user?.role, isImpersonating),
